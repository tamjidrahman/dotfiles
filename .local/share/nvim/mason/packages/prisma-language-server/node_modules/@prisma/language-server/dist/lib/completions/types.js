...
function getNativeTypes(schema, prismaType, onError) {
    let nativeTypes = (0, nativeTypes_1.default)(schema, (errorMessage) => {
        if (onError) {
...
function getSuggestionForNativeTypes(foundBlock, schema, wordsBeforePosition, onError) {
    const activeFeatureFlag = (0, ast_1.declaredNativeTypes)(schema, onError);
    if (
...
function getSuggestionsForFieldTypes(schema, position, currentLineUntrimmed) {
    const suggestions = [];
    ...
    const modelNames = datasourceProvider === 'mongodb'
        ? (0, ast_1.getAllRelationNames)(schema, constants_1.relationNamesMongoDBRegexFilter)
        : (0, ast_1.getAllRelationNames)(schema, constants_1.relationNamesRegexFilter);
