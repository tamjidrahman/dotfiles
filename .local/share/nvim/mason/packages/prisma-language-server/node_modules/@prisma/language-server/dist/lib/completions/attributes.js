...
const filterContextBlockAttributes = (schema, suggestions) => {
 ...
 */
function filterSuggestionsForBlock(suggestions, block, schema) {
 ...
 */
function getSuggestionForBlockAttribute(block, schema) {
    if (!['model', 'view'].includes(block.type)) {
        return [];
    }
    const suggestions = filterSuggestionsForBlock((0, klona_1.klona)(blockAttributes), block, schema);
 ...
 * @returns
 */
function getSuggestionForFieldAttribute(block, currentLine, schema, wordsBeforePosition, onError) {
    ...
    if (wordsBeforePosition.length >= 2) {
        const datasourceName = (0, ast_1.getFirstDatasourceName)(schema);
        const prismaType = wordsBeforePosition[1];
        const nativeTypeSuggestions = (0, types_1.getNativeTypes)(schema, prismaType, onError);
        if (datasourceName) {
    ...
    const datamodelBlock = (0, ast_1.getDatamodelBlock)(fieldType, schema);
