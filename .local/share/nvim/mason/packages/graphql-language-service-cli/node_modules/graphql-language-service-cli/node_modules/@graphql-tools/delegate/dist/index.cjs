...
function mergeFields(mergedTypeInfo, object, sourceSubschema, context, info) {
  ...
  const res$ = delegationMaps.reduce(
    (prev, delegationMap) => {
      function executeFn() {
  ...
  if (resolverResult instanceof Error || resolverResult == null) {
    ...
    for (const [responseKey, fieldNodes] of fields) {
      ...
      if (resolverResult instanceof graphql.GraphQLError) {
        nullResult[responseKey] = utils.relocatedError(resolverResult, combinedPath);
  ...
  for (const [subschema, selectionSet] of delegationMap) {
    ...
    if (resolver) {
      try {
        ...
        if (utils.isPromise(resolverResult$)) {
          jobs.push(
            resolverResult$.then(
...
function updateArguments(targetField, argumentNodeMap, variableDefinitionMap, variableValues, newArgs) {
  const generateVariableName = utils.createVariableNameGenerator(
    variableDefinitionMap
  );
...
function finalizeSelectionSet(schema, type, validFragments, selectionSet, onOverlappingAliases) {
  const usedFragments = [];
  const usedVariables = [];
  const typeInfo = graphql.versionInfo.major < 16 ? new graphql.TypeInfo(schema, void 0, type) : new graphql.TypeInfo(schema, type);
...
function createRequest({
  ...
  if (sourceSchema != null && variableDefinitions != null) {
    for (const def of variableDefinitions) {
      ...
      const serializedValue = utils.serializeInputValue(
        varType,
    ...
    if (argumentNodeMap[argName] === void 0) {
      ...
      if (defaultValue !== void 0) {
        utils.updateArgument(
          ...
          generateVariableName(argName),
          sourceArgType,
          utils.serializeInputValue(sourceArgType, defaultValue)
        );
      }
    }
  }
...
function handleResult(parent, responseKey, context, info) {
  ...
  const leftOver = getPlanLeftOverFromParent(parent);
  if (leftOver) {
    if (utils.isPromise(resolvedData$)) {
...
const createDefaultExecutor = utils.memoize1(function createDefaultExecutor2(schema) {
  return function defaultExecutor(request) {
    return executor.normalizedExecutor({
      schema,
...
const extractUnavailableFieldsFromSelectionSet = utils.memoize4(
  function extractUnavailableFieldsFromSelectionSet2(schema, fieldType, fieldSelectionSet, shouldAdd) {
    ...
    if (graphql.isUnionType(fieldType)) {
      ...
      for (const type of fieldType.getTypes()) {
        ...
        unavailableSelections2.push(
          ...extractUnavailableFieldsFromSelectionSet2(
            schema,
            type,
    ...
    for (const selection of fieldSelectionSet.selections) {
      if (selection.kind === graphql.Kind.FIELD) {
        ...
        if (subFieldName === fieldType.name || (graphql.isObjectType(subFieldType) || graphql.isInterfaceType(subFieldType)) && graphql.isAbstractType(fieldType) && schema.isSubType(fieldType, subFieldType)) {
          const unavailableFields = extractUnavailableFieldsFromSelectionSet2(
            schema,
            subFieldType,
            selection.selectionSet,
            shouldAdd
...
const extractUnavailableFields = utils.memoize4(
  function extractUnavailableFields2(schema, field, fieldNode, shouldAdd) {
...
function getPathWithFieldNames(node, path) {
  ...
  for (const segment of path) {
    currentNode = currentNode[segment];
    if (!isArrayOfSelectionNodes(currentNode) && currentNode.kind === graphql.Kind.FIELD) {
...
function subtractSelectionSets(selectionSetA, selectionSetB) {
  return graphql.visit(selectionSetA, {
    [graphql.Kind.FIELD]: {
